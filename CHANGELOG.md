...
- **Switching inverters fails to reconnect**: The poll loop captured the
  settings version *after* the poll operation, creating a race condition.
  If the API bumped the version during the poll (1-3 second window), the
